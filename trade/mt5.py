...
def options_to_watch(symbols_mt5, options_watch, venc_call, venc_put):
    ...
    for watch in options_watch:
        for symbol in symbols_mt5:
            if symbol.name.startswith(watch + venc_call) or\
                symbol.name.startswith(watch + venc_put):
...
def set_market_watch():
    symbols_mt5 = get_mt5_symbols()
    symbols_watch = [
    'PETR4', 'VALE3', 'ITUB4', 'ITSA4', 'BBDC4', 'BBAS3', 'BOVA11'
    ]
    options_watch = [
    'PETR', 'VALE', 'ITUB', 'ITSA', 'BBDC', 'BBAS', 'BOVA'
    ]
